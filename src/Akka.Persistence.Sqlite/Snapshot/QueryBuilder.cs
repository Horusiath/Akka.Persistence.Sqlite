...
namespace Akka.Persistence.Sqlite.Snapshot
{
    internal class QueryBuilder : ISnapshotQueryBuilder
    {
        ...
        public QueryBuilder(SqliteSnapshotSettings settings)
        {
            ...
            _insertSql = string.Format(@"INSERT INTO {0} (persistence_id, sequence_nr, created_at, snapshot_type, snapshot) VALUES (@PersistenceId, @SequenceNr, @Timestamp, @SnapshotType, @Snapshot)", settings.TableName);
            _selectSql = string.Format(@"SELECT persistence_id, sequence_nr, created_at, snapshot_type, snapshot FROM {0} WHERE persistence_id = ? ", settings.TableName);
        }
        ...
        public DbCommand InsertSnapshot(SnapshotEntry entry)
        {
            var sqlCommand = new SQLiteCommand(_insertSql)
            {
                Parameters =
                {
                    ...
                    new SQLiteParameter("@SnapshotType", DbType.String, entry.SnapshotType.Length) { Value = entry.SnapshotType },
